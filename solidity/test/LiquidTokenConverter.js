...
contract('LiquidTokenConverter:', accounts => {
    ...
    it('verifies that rateAndFee returns a valid amount', async () => {
        ...
        let returnAmount = (await converter.rateAndFee.call(reserveToken.address, tokenAddress, 500))[0];
    ...
    it('should throw when attempting to get the purchase rate while the converter is not active', async () => {
        ...
        await utils.catchRevert(converter.rateAndFee.call(reserveToken.address, tokenAddress, 500));
    });
    ...
    it('should throw when attempting to convert when the return is smaller than the minimum requested amount', async () => {
        ...
        await utils.catchRevert(convert([reserveToken.address, tokenAddress, tokenAddress], 500, 2000));
    });
    ...
    it('should throw when attempting to buy while the converter is not active', async () => {
        await initConverter(accounts, false);
        await approve(reserveToken, accounts[0], bancorNetwork.address, 500);
