...
contract('BancorNetworkWithOldEtherToken', accounts => {
    ...
    it('verifies the caller balances after selling directly for ether with a single converter', async () => {
        ...
        let prevETHBalance = await balance.current(accounts[0]);

        let res = await bancorNetwork.convert(smartToken1SellPath, 10000, 1);
        let newETHBalance = await balance.current(accounts[0]);
        let newTokenBalance = await smartToken1.balanceOf.call(accounts[0]);

        let transaction = await web3.eth.getTransaction(res.tx);
        let transactionCost = new BN(transaction.gasPrice).mul(new BN(res.receipt.cumulativeGasUsed));
    ...
    it('should revert when attempting register ether token with invalid address', async () => {
        let bancorNetwork1 = await BancorNetwork.new(contractRegistry.address);
        await utils.catchRevert(bancorNetwork1.registerEtherToken(constants.ZERO_ADDRESS, true));
    });
    ...
    it('verifies that getReturnByPath returns the correct amount for selling the smart token', async () => {
        ...
        await smartToken1.approve(bancorNetwork.address, 100, { from: accounts[1] });
        let balanceBeforeTransfer = await balance.current(accounts[1]);
        let res = await bancorNetwork.convert(smartToken1SellPath, 100, 1, { from: accounts[1] });
        let transaction = await web3.eth.getTransaction(res.tx);
        let transactionCost = new BN(transaction.gasPrice).mul(new BN(res.receipt.cumulativeGasUsed));
        let balanceAfterTransfer = await balance.current(accounts[1]);
    ...
    it('verifies that convertFor2 transfers the converted amount correctly', async () => {
        let balanceBeforeTransfer = await smartToken1.balanceOf.call(accounts[1]);
        await bancorNetwork.convertFor2(smartToken1BuyPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0, { value: 10000 });
        let balanceAfterTransfer = await smartToken1.balanceOf.call(accounts[1]);
        assert.isAbove(balanceAfterTransfer.toNumber(), balanceBeforeTransfer.toNumber(), 'amount transfered');
    });
    ...
    it('verifies that convert2 transfers the converted amount correctly', async () => {
        let balanceBeforeTransfer = await smartToken1.balanceOf.call(accounts[1]);
        await bancorNetwork.convert2(smartToken1BuyPath, 10000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 10000 });
        let balanceAfterTransfer = await smartToken1.balanceOf.call(accounts[1]);
        assert.isAbove(balanceAfterTransfer.toNumber(), balanceBeforeTransfer.toNumber(), 'amount transfered');
    });
    ...
    it('verifies claimAndConvertFor2 with a path that starts with a smart token and ends with another smart token', async () => {
        ...
        let path = [smartToken4.address, smartToken3.address, smartToken3.address, smartToken2.address, smartToken2.address];
        let balanceBeforeTransfer = await smartToken2.balanceOf.call(accounts[1]);
        await bancorNetwork.claimAndConvertFor2(path, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0);
        let balanceAfterTransfer = await smartToken2.balanceOf.call(accounts[1]);
        assert.isAbove(balanceAfterTransfer.toNumber(), balanceBeforeTransfer.toNumber(), 'amount transfered');
    });

    it('verifies that convertFor2 returns a valid amount when buying the smart token', async () => {
        let amount = await bancorNetwork.convertFor2.call(smartToken1BuyPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0, { value: 10000 });
        assert.isAbove(amount.toNumber(), 0, 'amount converted');
    });

    it('verifies that convert2 returns a valid amount when buying the smart token', async () => {
        let amount = await bancorNetwork.convert2.call(smartToken1BuyPath, 10000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 10000 });
        assert.isAbove(amount.toNumber(), 0, 'amount converted');
    });

    it('verifies that convertFor2 returns a valid amount when converting from ETH to ERC20', async () => {
        let amount = await bancorNetwork.convertFor2.call(etherToErc20ConvertPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0, { value: 10000 });
        assert.isAbove(amount.toNumber(), 0, 'amount converted');
    });

    it('verifies that convert2 returns a valid amount when converting from ETH to ERC20', async () => {
        let amount = await bancorNetwork.convert2.call(etherToErc20ConvertPath, 10000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 10000 });
        assert.isAbove(amount.toNumber(), 0, 'amount converted');
    });

    it('should revert when calling convertFor2 with ether token but without sending ether', async () => {
        await utils.catchRevert(bancorNetwork.convertFor2(smartToken1BuyPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0));
    });

    it('should revert when calling convertFor2 with ether amount different than the amount sent', async () => {
        await utils.catchRevert(bancorNetwork.convertFor2.call(smartToken1BuyPath, 20000, 1, accounts[1], constants.ZERO_ADDRESS, 0, { value: 10000 }));
    });

    it('should revert when calling convertFor2 with invalid path', async () => {
        let invalidPath = [etherToken.address, smartToken1.address];
        await utils.catchRevert(bancorNetwork.convertFor2(invalidPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0, { value: 10000 }));
    });
    ...
    it('should revert when calling convertFor2 with invalid long path', async () => {
        ...
        await utils.catchRevert(bancorNetwork.convertFor2(longBuyPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0, { value: 10000 }));
    });

    it('should revert when calling convert2 with ether token but without sending ether', async () => {
        await utils.catchRevert(bancorNetwork.convert2(smartToken1BuyPath, 10000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1] }));
    });

    it('should revert when calling convert2 with ether amount different than the amount sent', async () => {
        await utils.catchRevert(bancorNetwork.convert2.call(smartToken1BuyPath, 20000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 10000 }));
    });

    it('should revert when calling convert2 with invalid path', async () => {
        let invalidPath = [etherToken.address, smartToken1.address];
        await utils.catchRevert(bancorNetwork.convert2(invalidPath, 10000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 10000 }));
    });
    ...
    it('should revert when calling convert2 with invalid long path', async () => {
        ...
        await utils.catchRevert(bancorNetwork.convert2(longBuyPath, 10000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 10000 }));
    });

    it('verifies that claimAndConvertFor2 transfers the converted amount correctly', async () => {
        await smartToken1.approve(bancorNetwork.address, 10000);
        let balanceBeforeTransfer = await smartToken3.balanceOf.call(accounts[1]);
        await bancorNetwork.claimAndConvertFor2(smartToken3BuyPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0);
        let balanceAfterTransfer = await smartToken3.balanceOf.call(accounts[1]);
        assert.isAbove(balanceAfterTransfer.toNumber(), balanceBeforeTransfer.toNumber(), 'amount transfered');
    });

    it('should revert when calling claimAndConvertFor2 without approval', async () => {
        await utils.catchRevert(bancorNetwork.claimAndConvertFor2(smartToken3BuyPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0));
    });

    it('verifies that claimAndConvert2 transfers the converted amount correctly', async () => {
        await smartToken1.approve(bancorNetwork.address, 10000);
        let balanceBeforeTransfer = await smartToken3.balanceOf.call(accounts[0]);
        await bancorNetwork.claimAndConvert2(smartToken3BuyPath, 10000, 1, constants.ZERO_ADDRESS, 0);
        let balanceAfterTransfer = await smartToken3.balanceOf.call(accounts[0]);
        assert.isAbove(balanceAfterTransfer.toNumber(), balanceBeforeTransfer.toNumber(), 'amount transfered');
    });

    it('should revert when calling claimAndConvert2 without approval', async () => {
        await utils.catchRevert(bancorNetwork.claimAndConvert2(smartToken3BuyPath, 10000, 1, constants.ZERO_ADDRESS, 0));
    });
    ...
    it('verifies that getReturnByPath returns the correct amount for buying the smart token through multiple converters', async () => {
        let returnByPath = (await bancorNetwork.getReturnByPath.call(smartToken2BuyPath, 10000))[0];
        let balanceBeforeTransfer = await smartToken2.balanceOf.call(accounts[1]);
        await bancorNetwork.convertFor2(smartToken2BuyPath, 10000, 1, accounts[1], constants.ZERO_ADDRESS, 0, { value: 10000 });
        let balanceAfterTransfer = await smartToken2.balanceOf.call(accounts[1]);
        assert.equal(returnByPath, balanceAfterTransfer - balanceBeforeTransfer);
    });
    ...
    it('verifies that getReturnByPath returns the correct amount for buying the smart token through multiple converters', async () => {
        let returnByPath = (await bancorNetwork.getReturnByPath.call(smartToken2BuyPath, 10000))[0];
        let balanceBeforeTransfer = await smartToken2.balanceOf.call(accounts[1]);
        await bancorNetwork.convert2(smartToken2BuyPath, 10000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 10000 });
        let balanceAfterTransfer = await smartToken2.balanceOf.call(accounts[1]);
        assert.equal(returnByPath, balanceAfterTransfer - balanceBeforeTransfer);
    });
    ...
    it('should be able to convert2 from a non compliant erc-20 to another token', async () => {
        ...
        let path = [erc20Token.address, smartToken4.address, smartToken4.address];
        let prevBalance = await smartToken4.balanceOf.call(accounts[0]);
        await bancorNetwork.convert2(path, 1000, 1, constants.ZERO_ADDRESS, 0);
        let postBalance = await smartToken4.balanceOf.call(accounts[0]);
    ...
    it('verifies that convert2 with a single converter results in increased balance for the buyer', async () => {
        ...
        let res = await bancorNetwork.convert2(smartToken1BuyPath, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 100 });
        let newBalance = await smartToken1.balanceOf.call(accounts[1]);
    ...
    it('verifies that convert2 with multiple converters results in increased balance for the buyer', async () => {
        ...
        let res = await bancorNetwork.convert2(smartToken2BuyPath, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 100 });
        let newBalance = await smartToken2.balanceOf.call(accounts[1]);
    ...
    it('verifies that convert2 with minimum return equal to the full expected return amount results in the exact increase in balance for the buyer', async () => {
        ...
        await bancorNetwork.convert2(smartToken2BuyPath, 100000, token2Return, constants.ZERO_ADDRESS, 0, { value: 100000 });
        let newBalance = await smartToken2.balanceOf.call(accounts[0]);
    ...
    it('should revert when attempting to convert2 and the return amount is lower than the given minimum', async () => {
        await utils.catchRevert(bancorNetwork.convert2(smartToken2BuyPath, 100, 1000000, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 100 }));
    });
    ...
    it('should revert when attempting to convert2 and passing an amount higher than the ETH amount sent with the request', async () => {
        await utils.catchRevert(bancorNetwork.convert2(smartToken2BuyPath, 100001, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 100000 }));
    });
    ...
    it('verifies the caller balances after selling directly for ether with a single converter', async () => {
        ...
        let res = await bancorNetwork.convert2(smartToken1SellPath, 10000, 1, constants.ZERO_ADDRESS, 0);
        let newETHBalance = await balance.current(accounts[0]);
        let newTokenBalance = await smartToken1.balanceOf.call(accounts[0]);
    ...
    it('verifies the caller balances after selling directly for ether with multiple converters', async () => {
        ...
        let res = await bancorNetwork.convert2(smartToken2SellPath, 10000, 1, constants.ZERO_ADDRESS, 0);
        let newETHBalance = await balance.current(accounts[0]);
        let newTokenBalance = await smartToken2.balanceOf.call(accounts[0]);
    ...
    it('should revert when attempting to sell directly for ether and the return amount is lower than the given minimum', async () => {
        await smartToken2.approve(bancorNetwork.address, 10000);
        await utils.catchRevert(bancorNetwork.convert2(smartToken2SellPath, 10000, 20000, constants.ZERO_ADDRESS, 0));
        await smartToken2.approve(bancorNetwork.address, 0);
    });
    ...
    it('verifies that getReturnByPath returns the correct amount for cross reserve conversion', async () => {
        await bancorNetwork.convert2([etherToken.address, smartToken1.address, smartToken1.address], 1000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 1000 });
        await smartToken1.approve(bancorNetwork.address, 100, { from: accounts[1] });
        let path = [smartToken1.address, smartToken2.address, smartToken3.address];
        let returnByPath = (await bancorNetwork.getReturnByPath.call(path, 100))[0];
        let balanceBeforeTransfer = await smartToken3.balanceOf.call(accounts[1]);
        await bancorNetwork.convert2(path, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1] });
        let balanceAfterTransfer = await smartToken3.balanceOf.call(accounts[1]);
        assert.equal(returnByPath, balanceAfterTransfer - balanceBeforeTransfer);
    });

    it('verifies that getReturnByPath returns the correct amount for selling the smart token', async () => {
        await bancorNetwork.convert2(smartToken1BuyPath, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 100 });
        await smartToken1.approve(bancorNetwork.address, 100, { from: accounts[1] });
        let returnByPath = (await bancorNetwork.getReturnByPath.call(smartToken1SellPath, 100))[0];
        let balanceBeforeTransfer = await balance.current(accounts[1]);
        let res = await bancorNetwork.convert2(smartToken1SellPath, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1] });
        let transaction = await web3.eth.getTransaction(res.tx);
        let transactionCost = new BN(transaction.gasPrice).mul(new BN(res.receipt.cumulativeGasUsed));
        let balanceAfterTransfer = await balance.current(accounts[1]);
        assert.equal(returnByPath.toNumber(), balanceAfterTransfer.minus(balanceBeforeTransfer).plus(transactionCost).toNumber());
    ...
    it('verifies that getReturnByPath returns the correct amount for selling the smart token through multiple converters', async () => {
        await bancorNetwork.convert2(smartToken2BuyPath, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 100 });
        await smartToken2.approve(bancorNetwork.address, 100, { from: accounts[1] });
        let returnByPath = (await bancorNetwork.getReturnByPath.call(smartToken2SellPath, 100))[0];
        let balanceBeforeTransfer = await balance.current(accounts[1]);
        let res = await bancorNetwork.convert2(smartToken2SellPath, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1] });
        let transaction = await web3.eth.getTransaction(res.tx);
        let transactionCost = new BN(transaction.gasPrice).mul(new BN(res.receipt.cumulativeGasUsed));
        let balanceAfterTransfer = await balance.current(accounts[1]);
        assert.equal(returnByPath.toNumber(), balanceAfterTransfer.minus(balanceBeforeTransfer).plus(transactionCost).toNumber());
    ...
    it('verifies that getReturnByPath returns the correct amount for selling the smart token with a long conversion path', async () => {
        await bancorNetwork.convert2([etherToken.address, smartToken1.address, smartToken1.address, smartToken2.address, smartToken3.address], 1000, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1], value: 1000 });
        await smartToken3.approve(bancorNetwork.address, 100, { from: accounts[1] });
        let path = [smartToken3.address, smartToken2.address, smartToken2.address, smartToken2.address, smartToken1.address, smartToken1.address, etherToken.address];
        let returnByPath = (await bancorNetwork.getReturnByPath.call(path, 100))[0];
        let balanceBeforeTransfer = await balance.current(accounts[1]);
        let res = await bancorNetwork.convert2(path, 100, 1, constants.ZERO_ADDRESS, 0, { from: accounts[1] });
        let transaction = await web3.eth.getTransaction(res.tx);
        let transactionCost = new BN(transaction.gasPrice).mul(new BN(res.receipt.cumulativeGasUsed));
        let balanceAfterTransfer = await balance.current(accounts[1]);
        assert.equal(returnByPath.toNumber(), balanceAfterTransfer.minus(balanceBeforeTransfer).plus(transactionCost).toNumber());
    ...
    it('should revert when calling setMaxAffiliateFee with a non-owner or an illegal value', async () => {
