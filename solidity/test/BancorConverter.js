...
contract('BancorConverter', accounts => {
    ...
    it('verifies that getReturn returns the same amount as buy -> sell when converting between 2 reserves', async () => {
        ...
        let purchaseAmount = await getConversionAmount(watcher);
        await approve(token, accounts[0], bancorNetwork.address, purchaseAmount);
        await bancorNetwork.convert2([tokenAddress, tokenAddress, reserveToken2.address], purchaseAmount, 1, utils.zeroAddress, 0);
        let saleAmount = await getConversionAmount(watcher);
    ...
    it('verifies that convert2 returns valid amount and fee after converting', async () => {
        ...
        await converter.setConversionFee(3000);
        await approve(reserveToken, accounts[0], bancorNetwork.address, 500);
        await bancorNetwork.convert2([reserveToken.address, tokenAddress, reserveToken2.address], 500, 1, utils.zeroAddress, 0);
        let events = await watcher.get();
        assert(events.length > 0);
        assert(events[0].args._return.equals(1167), events[0].args._conversionFee.equals(8));
    });
    ...
    it('should throw when attempting to convert2 with an invalid source token adress', async () => {
        ...
        await utils.catchRevert(bancorNetwork.convert2([utils.zeroAddress, tokenAddress, reserveToken2.address], 500, 1, utils.zeroAddress, 0));
    });
    ...
    it('should throw when attempting to convert2 with an invalid target token address', async () => {
        ...
        await utils.catchRevert(bancorNetwork.convert2([reserveToken.address, tokenAddress, utils.zeroAddress], 500, 1, utils.zeroAddress, 0));
    });
    ...
    it('should throw when attempting to convert2 with 0 minimum requested amount', async () => {
        ...
        await utils.catchRevert(bancorNetwork.convert2([reserveToken.address, tokenAddress, reserveToken2.address], 500, 0, utils.zeroAddress, 0));
    });
    ...
    it('should throw when attempting to convert2 when the return is smaller than the minimum requested amount', async () => {
        ...
        await utils.catchRevert(bancorNetwork.convert2([reserveToken.address, tokenAddress, reserveToken2.address], 500, 2000, utils.zeroAddress, 0));
    });
