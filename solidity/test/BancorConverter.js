...
contract('BancorConverter', accounts => {
    ...
    it('verifies the converter data after construction', async () => {
        let converter = await BancorConverter.new(tokenAddress, contractFeatures.address, converterExtensionsAddress, 0, '0x0', 0);
        let token = await converter.token.call();
        assert.equal(token, tokenAddress);
        let features = await converter.features.call();
        assert.equal(features, contractFeatures.address);
    ...
    it('should throw when attempting to construct a converter with no token', async () => {
        try {
            await BancorConverter.new('0x0', contractFeatures.address, converterExtensionsAddress, 0, '0x0', 0);
    ...
    it('should throw when attempting to construct a converter with no converter extensions', async () => {
        try {
            await BancorConverter.new(tokenAddress, contractFeatures.address, '0x0', 0, '0x0', 0);
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('should throw when attempting to construct a converter with invalid max fee', async () => {
        try {
            await BancorConverter.new(tokenAddress, contractFeatures.address, converterExtensionsAddress, 1000000000, '0x0', 0);
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('verifies the first connector when provided at construction time', async () => {
        let converter = await BancorConverter.new(tokenAddress, contractFeatures.address, converterExtensionsAddress, 0, connectorTokenAddress, 200000);
    ...
    it('should throw when attempting to construct a converter with a connector with invalid weight', async () => {
        try {
            await BancorConverter.new(tokenAddress, contractFeatures.address, converterExtensionsAddress, 0, connectorTokenAddress, 1000001);
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('should throw when the attempts update the converter extensions contract address with an invalid address', async () => {
        let converter = await BancorConverter.new(tokenAddress, contractFeatures.address, converterExtensionsAddress, 0, '0x0', 0);

        try {
            await converter.setExtensions('0x0');
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('should throw when the owner attempts update the converter extensions contract address with the converter address', async () => {
        let converter = await BancorConverter.new(tokenAddress, contractFeatures.address, converterExtensionsAddress, 0, '0x0', 0);

        try {
            await converter.setExtensions(converter.address);
            assert(false, "didn't throw");
        }
        catch (error) {
            return utils.ensureException(error);
    ...
    it('verifies the owner can update the fee', async () => {
        let converter = await BancorConverter.new(tokenAddress, contractFeatures.address, converterExtensionsAddress, 200000, '0x0', 0);
    ...
    it('should throw when attempting to add a connector when the converter is active', async () => {
        let token = await SmartToken.new('Token1', 'TKN1', 2);
        let converter = await BancorConverter.new(token.address, contractFeatures.address, converterExtensionsAddress, 0, '0x0', 0);
