module.exports.CONTRACT_REGISTRY       = web3.fromAscii("ContractRegistry");
module.exports.BANCOR_NETWORK          = web3.fromAscii("BancorNetwork");
module.exports.BANCOR_FORMULA          = web3.fromAscii("BancorFormula");
module.exports.CONVERTER_FACTORY       = web3.fromAscii("ConverterFactory");
module.exports.CONVERSION_PATH_FINDER  = web3.fromAscii("ConversionPathFinder");
module.exports.CONVERTER_UPGRADER      = web3.fromAscii("BancorConverterUpgrader");
module.exports.CONVERTER_REGISTRY      = web3.fromAscii("BancorConverterRegistry");
module.exports.CONVERTER_REGISTRY_DATA = web3.fromAscii("BancorConverterRegistryData");
module.exports.BNT_TOKEN               = web3.fromAscii("BNTToken");
module.exports.BANCOR_X                = web3.fromAscii("BancorX");
module.exports.BANCOR_X_UPGRADER       = web3.fromAscii("BancorXUpgrader");
