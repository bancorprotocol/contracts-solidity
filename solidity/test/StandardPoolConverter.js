...
describe('StandardPoolConverter', () => {
    ...
    it('verifies the TokenRateUpdate event after adding liquidity', async () => {
        ...
        const res = await converter.methods['addLiquidity(address[2],uint256[2],uint256)'](
    ...
    it('verifies the TokenRateUpdate event after removing liquidity', async () => {
        ...
        const res = await converter.methods['removeLiquidity(uint256,address[2],uint256[2])'](
            100,
            [reserveToken.address, reserveToken2.address],
            [MIN_RETURN, MIN_RETURN]
        );
        ...
        describe(`${isETHReserve === 0 ? '(with ERC20 reserves)' : '(with ETH reserve)'}:`, () => {
            ...
            it('should revert when attempting to add liquidity with insufficient funds', async () => {
                ...
                await expectRevert.unspecified(
                    converter.methods['addLiquidity(address[2],uint256[2],uint256)'](
                        [getReserve1Address(isETHReserve), reserveToken2.address],
                        [amount, 1000],
                        1,
                        { from: sender2, value }
            ...
            it('should revert when attempting to remove liquidity with insufficient funds', async () => {
                ...
                await converter.methods['removeLiquidity(uint256,address[2],uint256[2])'](5, [getReserve1Address(isETHReserve), reserveToken2.address], [1, 1], {
                    from: sender2
                });

                await expectRevert.unspecified(
                    converter.methods['removeLiquidity(uint256,address[2],uint256[2])'](600, [getReserve1Address(isETHReserve), reserveToken2.address], [1, 1], {
                        from: sender2
                    })
                );
            });
    ...
    describe('verifies that the maximum possible liquidity is added', () => {
        ...
        for (const [amount1, amount2] of amounts) {
            it(`addLiquidity(${[amount1, amount2]})`, async () => {
                ...
                await converter.methods['addLiquidity(address[2],uint256[2],uint256)']([reserveToken1.address, reserveToken2.address], [amount1, amount2], 1);
    ...
    describe('add/remove liquidity', () => {
        ...
        const test = async (hasETH) => {
            ...
            for (const supplyAmount of [1000000000, 1000000, 2000000, 3000000, 4000000]) {
                ...
                await converter.methods['addLiquidity(address[2],uint256[2],uint256)'](reserveTokens, reserveAmounts, MIN_RETURN, {
    ...
    describe('verifies that the network fee is transferred correctly via', () => {
                ...
                for (const conversionFeePercent of [0, 5, 10, 25, 75]) {
                    for (const networkFeePercent of [0, 5, 10, 25, 75, 100]) {
                        it(description('processNetworkFees when', initialBalance1, initialBalance2, conversionFeePercent, networkFeePercent), async () => {
                            ...
                            const expectedFeeBase = conversion.fee.divn(2).muln(networkFeePercent).divn(100 + networkFeePercent);
                ...
                for (const conversionFeePercent of [1, 2]) {
                    for (const networkFeePercent of [5, 10]) {
                        it(description('removeLiquidity when', initialBalance1, initialBalance2, conversionFeePercent, networkFeePercent), async () => {
                            ...
                            const expectedFeeBase = totalConversionFeeInPoolTokenUnits.divn(2).muln(networkFeePercent).divn(100 + networkFeePercent);
        ...
        async function addLiquidity(reserveToken1, reserveToken2, converter, reserveAmounts, verify = false) {
            ...
            await converter.methods['addLiquidity(address[2],uint256[2],uint256)'](reserveTokens, reserveAmounts, 1);
        }
        ...
        async function removeLiquidity(reserveToken1, reserveToken2, converter, supplyAmount, verify = false) {
            ...
            await converter.methods['removeLiquidity(uint256,address[2],uint256[2])'](supplyAmount, reserveTokens, [1, 1]);
        }
