...
const XTransferRerouter = ethers.getContractFactory('XTransferRerouter');
...
describe('XTransferRerouter', () => {
    ...
    it("verify that a user can't call rerouteTx when rerouting is disabled", async () => {
        const txRouter = await (await XTransferRerouter).deploy(false);
    ...
    it('verify that calling rerouteTx emits an event properly', async () => {
        const txRouter = await (await XTransferRerouter).deploy(true);
