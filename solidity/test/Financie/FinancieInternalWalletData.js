...
contract('FinancieInternalWalletData', (accounts) => {
    ...
    before(async () => {
        ...
        internalWalletData = await FinancieInternalWalletData.new(managedContracts.address, platformToken.address, currencyToken.address);
        console.log(internalWalletData.address);

        console.log('[FinancieInternalWallet]initialize');
        internalWallet = await FinancieInternalWallet.new("0xA0d6B46ab1e40BEfc073E510e92AdB88C0A70c5C", currencyToken.address, internalWalletData.address);
        await managedContracts.activateTargetContract(internalWallet.address, true);
        await managedContracts.activateTargetContract(accounts[0], true);
    ...
    it('setBalanceOfToken/getBalanceOfToken', async () => {
        assert.equal(await managedContracts.validTargetContract(internalWallet.address), true);
        assert.equal(await managedContracts.validTargetContract(accounts[0]), true);
        assert.equal(await managedContracts.validTargetContract(cardToken.address), true);


        await internalWalletData.setBalanceOfToken(cardToken.address, user_id, 2);
        console.log('setBalanceOfToken OK');

        let amount = await internalWalletData.getBalanceOfToken(cardToken.address, user_id);
    ...
    it('setHolderOfToken/getHolderOfToken', async () => {
        ...
        await internalWalletData.setHolderOfToken(cardToken.address, user_id, true);
        console.log('setHolderOfToken OK');

        let flg = await internalWalletData.getHolderOfToken(cardToken.address, user_id);
    ...
    it('setBidsOfAuctions/getBidsOfAuctions', async () => {
        ...
        await internalWalletData.setBidsOfAuctions(auction.address, user_id, 1000);
        console.log('setBidsOfAuctions OK');

        let amount = await internalWalletData.getBidsOfAuctions(auction.address, user_id);
    ...
    it('setTotalBidsOfAuctions/getTotalBidsOfAuctions', async () => {
        ...
        await internalWalletData.setTotalBidsOfAuctions(auction.address, 10000);
        console.log('setTotalBidsOfAuctions OK');

        let amount = await internalWalletData.getTotalBidsOfAuctions(auction.address);
    ...
    it('setRecvCardsOfAuctions/getRecvCardsOfAuctions', async () => {
        ...
        await internalWalletData.setRecvCardsOfAuctions(auction.address, 100000);
        console.log('setRecvCardsOfAuctions OK');

        let amount = await internalWalletData.getRecvCardsOfAuctions(auction.address);
