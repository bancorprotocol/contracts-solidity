...
contract('BancorConverterWithEthReserve', accounts => {
    ...
    it('verifies that getReturn returns the same amount as getCrossReserveReturn when converting between 2 reserves', async () => {
        let converter = await initConverter(accounts, true);
        let returnAmount = (await converter.getReturn.call(reserveToken.address, utils.zeroAddress, 500))[0];
        let returnAmount2 = (await converter.getCrossReserveReturn.call(reserveToken.address, utils.zeroAddress, 500, 0))[0];
    ...
    it('verifies that getCrossReserveReturn returns the same amount as converting between 2 reserves', async () => {
        let converter = await initConverter(accounts, true);
        let returnAmount = (await converter.getCrossReserveReturn.call(reserveToken.address, utils.zeroAddress, 500, 0))[0];
...
//        await await web3.eth.sendTransaction({from: accounts[0], to: accounts[9], value: 5000});
...
//        await await web3.eth.sendTransaction({from: accounts[9], to: accounts[0], value: token2Balance});
...
//        await await web3.eth.sendTransaction({from: accounts[0], to: accounts[9], value: 500000});
...
//        await await web3.eth.sendTransaction({from: accounts[0], to: accounts[9], value: 100});
