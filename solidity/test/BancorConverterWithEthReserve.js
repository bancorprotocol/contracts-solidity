...
contract('BancorConverterWithEthReserve', accounts => {
    ...
    it('verifies that 2 reserves are added correctly', async () => {
        ...
        await converter.addReserve(ETH_RESERVE, 200000);
        reserve = await converter.reserves.call(ETH_RESERVE);
        verifyReserve(reserve, 0, 200000, true);
    });
    ...
    it('should throw when attempting to retrieve the balance for a reserve that does not exist', async () => {
        ...
        await utils.catchRevert(converter.reserveBalance.call(ETH_RESERVE));
    });
    ...
    it('verifies that getReturn returns the same amount as buy -> sell when converting between 2 reserves', async () => {
        let converter = await initConverter(accounts, true);
        let returnAmount = (await converter.getReturn.call(reserveToken.address, ETH_RESERVE, 500))[0];
        ...
        let saleRes = await converter.convert(tokenAddress, ETH_RESERVE, purchaseAmount, 1);
    ...
    it('should succeed when attempting to get the return from ETH reserve to token', async () => {
        ...
        let [amount, fee] = await converter.getReturn.call(ETH_RESERVE, reserveToken.address, 500);
    ...
    it('should succeed when attempting to get the return from token to ETH reserve', async () => {
        ...
        let [amount, fee] = await converter.getReturn.call(reserveToken.address, ETH_RESERVE, 500);
    ...
    it('verifies that convert returns valid amount and fee after converting to ETH reserve', async () => {
        ...
        await converter.setConversionFee(3000);
        await reserveToken.approve(converter.address, 500);
        let response = await converter.convert(reserveToken.address, ETH_RESERVE, 500, 1);
        assert(response.logs.length > 0 && response.logs[0].event == 'Conversion');
    ...
    it('should throw when attempting to convert from ETH reserve to token', async () => {
        ...
        await utils.catchRevert(converter.convert(ETH_RESERVE, reserveToken.address, 500, 1));
    });
    ...
    it('should throw when attempting to convert with 0 minimum requested amount', async () => {
        ...
        await utils.catchRevert(converter.convert(reserveToken.address, ETH_RESERVE, 500, 0));
    });
    ...
    it('should throw when attempting to convert when the return is smaller than the minimum requested amount', async () => {
        ...
        await utils.catchRevert(converter.convert(reserveToken.address, ETH_RESERVE, 500, 2000));
    });
    ...
    it('verifies that getReturn returns the same amount as converting between 2 reserves', async () => {
        ...
        await reserveToken.approve(converter.address, 500);
        let convertRes = await converter.convert(reserveToken.address, ETH_RESERVE, 500, 1);
    ...
    it('verifies that fund gets the correct reserve balance amounts from the caller', async () => {
        ...
        let prevReserve2Balance = await converter.reserveBalance.call(ETH_RESERVE);
        ...
        let reserve2Balance = await converter.reserveBalance.call(ETH_RESERVE);
    ...
    it('verifies that getReturn returns the same amount as buy -> sell when converting between 2 reserves', async () => {
        ...
        let purchaseAmount = getConversionAmount(purchaseRes);
        let saleRes = await converter.convert2(tokenAddress, ETH_RESERVE, purchaseAmount, 1, utils.zeroAddress, 0);
        let saleAmount = getConversionAmount(saleRes);
    ...
    it('verifies that convert2 returns valid amount and fee after converting to ETH reserve', async () => {
        ...
        await converter.setConversionFee(3000);
        await reserveToken.approve(converter.address, 500);
        let response = await converter.convert2(reserveToken.address, ETH_RESERVE, 500, 1, utils.zeroAddress, 0);
        assert(response.logs.length > 0 && response.logs[0].event == 'Conversion');
        assert(response.logs[0].args._return.equals(1167), response.logs[0].args._conversionFee.equals(8));
    });
    ...
    it('should throw when attempting to convert2 from ETH reserve to token', async () => {
        ...
        await utils.catchRevert(converter.convert2(ETH_RESERVE, reserveToken.address, 500, 1, utils.zeroAddress, 0));
    });
    ...
    it('should throw when attempting to convert2 with 0 minimum requested amount', async () => {
        ...
        await utils.catchRevert(converter.convert2(reserveToken.address, ETH_RESERVE, 500, 0, utils.zeroAddress, 0));
    });
    ...
    it('should throw when attempting to convert2 when the return is smaller than the minimum requested amount', async () => {
        ...
        await utils.catchRevert(converter.convert2(reserveToken.address, ETH_RESERVE, 500, 2000, utils.zeroAddress, 0));
    });
    ...
    it('verifies that getReturn returns the same amount as converting between 2 reserves', async () => {
        ...
        await reserveToken.approve(converter.address, 500);
        let convertRes = await converter.convert2(reserveToken.address, ETH_RESERVE, 500, 1, utils.zeroAddress, 0);
        let returnAmount2 = getConversionAmount(convertRes);
