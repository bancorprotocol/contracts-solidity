...
const { ROLE_SUPERVISOR, ROLE_OWNER, ROLE_MANAGER, ROLE_SEEDER } = roles;
const { CONVERTER_FACTORY, CONVERTER_REGISTRY, CONVERTER_REGISTRY_DATA } = registry;
...
describe('StakingRewardsStore', () => {
    ...
    const getProviderRewards = async (provider, poolToken, reserveToken) => {
        const data = await store.providerRewards(provider, poolToken.address, reserveToken.address);
    ...
    describe('construction', () => {
        it('should properly initialize roles', async () => {
            const newStore = await TestStakingRewardsStore.new();

            expect(await newStore.getRoleMemberCount(ROLE_SUPERVISOR)).to.be.bignumber.equal(new BN(1));
            expect(await newStore.getRoleMemberCount(ROLE_OWNER)).to.be.bignumber.equal(new BN(0));
            expect(await newStore.getRoleMemberCount(ROLE_MANAGER)).to.be.bignumber.equal(new BN(0));
            expect(await newStore.getRoleMemberCount(ROLE_SEEDER)).to.be.bignumber.equal(new BN(0));
            ...
            expect(await newStore.hasRole(ROLE_SUPERVISOR, supervisor)).to.be.true();
            expect(await newStore.hasRole(ROLE_OWNER, supervisor)).to.be.false();
            expect(await newStore.hasRole(ROLE_MANAGER, supervisor)).to.be.false();
            expect(await newStore.hasRole(ROLE_SEEDER, supervisor)).to.be.false();
        });
    });
        ...
        context('manager', async () => {
            ...
            context('with a registered pool', async () => {
                ...
                it('should allow removing pools', async () => {
                    ...
                    expectEvent(res, 'PoolProgramRemoved', {
                        poolToken: poolToken.address
                    });
    ...
    describe('last claim times', () => {
        ...
        it('should allow to update last claim time', async () => {
            expect(await store.providerLastClaimTime(provider)).to.be.bignumber.equal(new BN(0));

            await setTime(now.add(new BN(1)));
            const res = await store.updateProviderLastClaimTime(provider, { from: owner });
            expect(await store.providerLastClaimTime(provider)).to.be.bignumber.equal(now);
            expectEvent(res, 'ProviderLastClaimTimeUpdated', {
                provider,
                claimTime: now
            });
            ...
            expectEvent(res2, 'ProviderLastClaimTimeUpdated', {
                provider,
                claimTime: now
            });
            expect(await store.providerLastClaimTime(provider)).to.be.bignumber.equal(now);
