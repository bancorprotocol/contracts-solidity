...
describe('VortexBurner', () => {
    ...
    describe('construction', () => {
        ...
        it('should revert if initialized with an invalid governance token governance address', async () => {
            await expect(
                Contracts.VortexBurner.deploy(networkToken.address, Constants.ZERO_ADDRESS, contractRegistry.address)
            ).to.be.revertedWith('ERR_INVALID_ADDRESS');
        });

        it('should revert if initialized with an invalid contract registry address', async () => {
            await expect(
                Contracts.VortexBurner.deploy(networkToken.address, govTokenGovernance.address, Constants.ZERO_ADDRESS)
            ).to.be.revertedWith('ERR_INVALID_ADDRESS');
        });
    });
    ...
    describe('burning event', () => {
        ...
        const getBalance = async (token: any, account: any) => {
        ...
        const seedNetworkFeeWallet = async (feeAmount: any) => {
        ...
        const getTokenAddresses = (testTokens: any) => {
            const selectedTokens = testTokens.map((symbol: any) => data[symbol]);
            return selectedTokens.map((tokenData: any) => {
        ...
        beforeEach(async () => {
            ...
            for (const [symbol, tokenData] of Object.entries(data)) {
                ...
                const tokenAddress = isETH ? Constants.NATIVE_TOKEN_ADDRESS : token.address;
