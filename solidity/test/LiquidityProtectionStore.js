...
contract('LiquidityProtectionStore', accounts => {
    ...
    describe('general verification', () => {
        it('should revert when a non owner attempts to withdraw tokens', async () => {
            ...
            await expectRevert(liquidityProtectionStore.withdrawTokens(erc20Token.address, accounts[0], 1, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        ...
        it('should revert when a non owner attempts to increase system balance', async () => {
            await expectRevert(liquidityProtectionStore.incSystemBalance(accounts[0], 1, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        ...
        it('should revert when a non owner attempts to decrease system balance', async () => {
            ...
            await expectRevert(liquidityProtectionStore.decSystemBalance(accounts[0], 1, { from: nonOwner }), 'ERR_ACCESS_DENIED');
    ...
    describe('whitelisted pools basic verification', () => {
        it('should revert when a non owner attempts to add a whitelisted pool', async () => {
            await expectRevert(liquidityProtectionStore.addPoolToWhitelist(poolToken, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        ...
        it('should revert when a non owner attempts to remove a whitelisted pool', async () => {
            ...
            await expectRevert(liquidityProtectionStore.removePoolFromWhitelist(poolToken, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        ...
        it('should revert when the owner attempts to add a whitelisted pool which is already whitelisted', async () => {
            await liquidityProtectionStore.addPoolToWhitelist(poolToken, { from: owner });
            await expectRevert(liquidityProtectionStore.addPoolToWhitelist(poolToken, { from: owner }), 'ERR_POOL_ALREADY_WHITELISTED');
        });

        it('should revert when the owner attempts to remove a whitelisted pool which is not yet whitelisted', async () => {
            await expectRevert(liquidityProtectionStore.removePoolFromWhitelist(poolToken, { from: owner }), 'ERR_POOL_NOT_WHITELISTED');
        });
    ...
    describe('protected liquidities basic verification', () => {
        it('should revert when a non owner attempts to add a protected-liquidity item', async () => {
            await expectRevert(liquidityProtectionStore.addProtectedLiquidity(provider, poolToken, reserveToken, 1, 2, 3, 4, 5, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        });

        it('should revert when a non owner attempts to update a protected-liquidity item', async () => {
            await liquidityProtectionStore.addProtectedLiquidity(provider, poolToken, reserveToken, 1, 2, 3, 4, 5, { from: owner });
            await expectRevert(liquidityProtectionStore.updateProtectedLiquidityAmounts(0, 6, 7, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        });

        it('should revert when a non owner attempts to remove a protected-liquidity item', async () => {
            await liquidityProtectionStore.addProtectedLiquidity(provider, poolToken, reserveToken, 1, 2, 3, 4, 5, { from: owner });
            await expectRevert(liquidityProtectionStore.removeProtectedLiquidity(0, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        });

        it('should succeed when the owner attempts to add a protected-liquidity item', async () => {
            const response = await liquidityProtectionStore.addProtectedLiquidity(provider, poolToken, reserveToken, 1, 2, 3, 4, 5, { from: owner });
            expect(await liquidityProtectionStore.totalProtectedPoolAmount(poolToken)).to.be.bignumber.equal('1');
            expect(await liquidityProtectionStore.totalProtectedReserveAmount(poolToken, reserveToken)).to.be.bignumber.equal('2');
            const expectedEvent = { _provider: provider, _poolToken: poolToken, _reserveToken: reserveToken, _poolAmount: '1', _reserveAmount: '2' };
        ...
        it('should succeed when the owner attempts to update a protected-liquidity item', async () => {
            ...
            expect(await liquidityProtectionStore.totalProtectedReserveAmount(poolToken, reserveToken)).to.be.bignumber.equal('4');
            const expectedEvent = { _provider: provider, _prevPoolAmount: '1', _prevReserveAmount: '2', _newPoolAmount: '3', _newReserveAmount: '4' };
        ...
        it('should succeed when the owner attempts to remove a protected-liquidity item', async () => {
            ...
            expect(await liquidityProtectionStore.totalProtectedReserveAmount(poolToken, reserveToken)).to.be.bignumber.equal('0');
            const expectedEvent = { _provider: provider, _poolToken: poolToken, _reserveToken: reserveToken, _poolAmount: '1', _reserveAmount: '2' };
    ...
    describe('locked balances basic verification', () => {
        it('should revert when a non owner attempts to add a locked balance', async () => {
            await expectRevert(liquidityProtectionStore.addLockedBalance(provider, 1, 2, { from: nonOwner }), 'ERR_ACCESS_DENIED');
        ...
        it('should revert when a non owner attempts to remove a locked balance', async () => {
            ...
            await expectRevert(liquidityProtectionStore.removeLockedBalance(provider, 0, { from: nonOwner }), 'ERR_ACCESS_DENIED');
    ...
    describe('locked balances range verification', () => {
        it('should revert when start-index is equal to end-index', async () => {
            ...
            await expectRevert(liquidityProtectionStore.lockedBalanceRange(provider, 0, 0, { from: owner }), 'ERR_INVALID_INDICES');
        });

        it('should revert when start-index is larger than end-index', async () => {
            for (let amount = 1; amount <= 10; amount++)
                await liquidityProtectionStore.addLockedBalance(provider, amount, 1, { from: owner });
            await expectRevert(liquidityProtectionStore.lockedBalanceRange(provider, 1, 0, { from: owner }), 'ERR_INVALID_INDICES');
        });
        ...
        it('should succeed when start-index is smaller than end-index', async () => {
            ...
            for (let i = 0; i < range.length; i++)
                expect(range[i][0]).to.be.bignumber.equal(new BN(i + 3));
        });
        ...
        it('should succeed when end-index is larger than the total number of items', async () => {
            ...
            for (let i = 0; i < range.length; i++)
                expect(range[i][0]).to.be.bignumber.equal(new BN(i + 8));
        });
    });
    ...
    describe('whitelisted pools advanced verification', () => {
        const removeAllOneByOne = async (direction) => {
            ...
            for (const account of accounts)
                await liquidityProtectionStore.addPoolToWhitelist(account);
    ...
    describe('protected liquidities advanced verification', () => {
        const removeAllOneByOne = async (direction) => {
            console.log(`adding ${accounts.length} items...`);
            for (const account of accounts)
                await liquidityProtectionStore.addProtectedLiquidity(provider, account, DUMMY_ADDRESS, 1, 2, 3, 4, 5, { from: owner });
            for (let items = accounts.slice(); items.length > 0; items.length--) {
