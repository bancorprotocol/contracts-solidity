...
def formulaTest(x):
    resultSolidityPort = FormulaSolidityPort.lambertPos3(x)
...
for n in range(size):
    x = random.randrange(FormulaSolidityPort.LAMBERT_POS2_MAXVAL + 1, FormulaSolidityPort.LAMBERT_POS3_MAXVAL + 1)
    ratio = formulaTest(x)
