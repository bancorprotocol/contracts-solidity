...
'''
def balancedWeights(_primaryReserveStakedBalance, _primaryReserveBalance, _secondaryReserveBalance, _reserveRateNumerator, _reserveRateDenominator):
    if (_primaryReserveStakedBalance == _primaryReserveBalance):
        require(_primaryReserveStakedBalance > 0 or _secondaryReserveBalance > 0, "ERR_INVALID_RESERVE_BALANCE");
    else:
...
def lambertPos1(_x):
    xi = _x;
    res = (FIXED_1 - _x) * 0xde1bc4d19efcac82445da75b00000000; # x^(1-1) * (34! * 1^(1-1) / 1!) - x^(2-1) * (34! * 2^(2-1) / 2!)
...
'''
def lambertPos2(_x):
    x = _x - LAMBERT_CONV_RADIUS - 1;
    i = x // LAMBERT_POS2_SAMPLE;
...
'''
def lambertPos3(_x):
    l1 = optimalLog(_x) if _x < OPT_LOG_MAX_VAL else generalLog(_x);
    l2 = optimalLog(l1) if l1 < OPT_LOG_MAX_VAL else generalLog(l1);
...
'''
def safeFactors(_a, _b):
    if (_a <= FIXED_2 and _b <= FIXED_2):
        return (_a, _b);
    if (_a < FIXED_2):
        return (_a * FIXED_2 // _b, FIXED_2);
    if (_b < FIXED_2):
        return (FIXED_2, _b * FIXED_2 // _a);
    c = _a if _a > _b else _b;
    n = floorLog2(c // FIXED_1);
    return (_a >> n, _b >> n);

'''
    @dev computes "MAX_WEIGHT * a / (a + b)" and "MAX_WEIGHT * b / (a + b)".
'''
def normalizedWeights(_a, _b):
    if (_a <= _b):
        return accurateWeights(_a, _b);
    (y, x) = accurateWeights(_b, _a);
    return (x, y);

'''
    @dev computes "MAX_WEIGHT * a / (a + b)" and "MAX_WEIGHT * b / (a + b)", assuming that "a <= b".
'''
def accurateWeights(_a, _b):
    if (_a > MAX_UNF_WEIGHT):
        c = _a // (MAX_UNF_WEIGHT + 1) + 1;
        _a //= c;
        _b //= c;
    x = roundDiv(_a * MAX_WEIGHT, add(_a, _b));
    y = MAX_WEIGHT - x;
    return (x, y);
