...
contract LiquidTokenConverter is ConverterBase {
    ...
    ) public ConverterBase(_token, _registry, _maxConversionFee) {}
    ...
    function targetAmountAndFee(
        ...
        if (_sourceToken == IDSToken(address(anchor)) && reserves[_targetToken].isSet) return saleTargetAmount(_amount);
    ...
    function doConvert(
        ...
        emit TokenRateUpdate(
            ...
            reserveToken,
            reserveBalance(reserveToken).mul(PPM_RESOLUTION),
            totalSupply.mul(reserveWeight)
        );
    ...
    function buy(
        ...
        if (reserveToken == ETH_RESERVE_ADDRESS) require(msg.value == _amount, "ERR_ETH_AMOUNT_MISMATCH");
            ...
            require(
                msg.value == 0 && reserveToken.balanceOf(address(this)).sub(reserveBalance(reserveToken)) >= _amount,
                "ERR_INVALID_AMOUNT"
            );
