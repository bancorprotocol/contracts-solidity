...
contract LiquidityPoolV2Converter is LiquidityPoolConverter {
        ...
        validAddress(address(_secondaryReserveOracle))
    {
        ...
        require(
            oracleWhitelist.isWhitelisted(address(_primaryReserveOracle)) &&
        ...
        LiquidityPoolV2ConverterCustomFactory customFactory = LiquidityPoolV2ConverterCustomFactory(
            address(IConverterFactory(addressOf(CONVERTER_FACTORY)).customFactories(converterType()))
        );
     ...
     */
    function setMaxStakedBalances(uint256 _reserve1MaxStakedBalance, uint256 _reserve2MaxStakedBalance)
        public
        ownerOnly
    {
    ...
    function doConvert(
    ...
    ) internal override active validReserve(_sourceToken) validReserve(_targetToken) returns (uint256) {
        ...
        (uint256 amount, uint256 standardFee, uint256 totalFee) = targetAmountAndFees(
            _sourceToken,
            _targetToken,
            sourceTokenWeight,
            targetTokenWeight,
        ...
        returns (uint256)
    {
        ...
        require(
            _reserveToken == ETH_RESERVE_ADDRESS ? msg.value == _amount : msg.value == 0,
        ...
        if (maxStakedBalanceEnabled) {
            require(
                maxStakedBalances[_reserveToken] == 0 ||
                    initialStakedBalance.add(_amount) <= maxStakedBalances[_reserveToken],
        ...
        if (_reserveToken != ETH_RESERVE_ADDRESS) safeTransferFrom(_reserveToken, msg.sender, address(this), _amount);
        ...
        emit LiquidityAdded(
            ...
            _reserveToken,
            _amount,
            initialStakedBalance.add(_amount),
            poolTokenSupply.add(poolTokenAmount)
        );
    ...
    function rebalance() private {
        (reserves[primaryReserveToken].weight, reserves[secondaryReserveToken].weight) = effectiveReserveWeights(
            externalRate
        );
    }
    ...
    function effectiveReserveWeights(Fraction memory _rate) private view returns (uint32, uint32) {
        ...
        // get the new weights
        return
            IBancorFormula(addressOf(BANCOR_FORMULA)).balancedWeights(
     ...
     */
    function dispatchTokenRateUpdateEvent(
        IERC20Token _token1,
        IERC20Token _token2,
        uint32 _token1Weight,
        uint32 _token2Weight
    ) private {
