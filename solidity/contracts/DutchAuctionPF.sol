...
contract DutchAuctionPF {
    ...
    uint constant public token_claim_waiting_period = 7 minutes;

    // Bid value over which the address has to be whitelisted
    // At deployment moment, less than 1k$
    uint constant public bid_threshold = 1000000 ether;
    ...
    function setup(address _core_address, address _token_address, address _pf_token_address) public isOwner atStage(Stages.AuctionDeployed) {
        require(_core_address != 0x0);
        require(_token_address != 0x0);
        require(_pf_token_address != 0x0);
        core = IFinancieCore(_core_address);
        token = ERC20Token(_token_address);
        pfToken = ERC20Token(_pf_token_address);
        ...
        atStage(Stages.AuctionStarted)
    {
        ...
        require(amount <= missing_funds);

        bids[msg.sender] += amount;
        received_wei += amount;

        // Send bid amount to wallet
        assert(pfToken.transferFrom(msg.sender, this, amount));
        assert(pfToken.transfer(wallet_address, amount));
        ...
        returns (bool)
    {
        ...
        core.notifyWithdrawalCards(msg.sender, address(token), num);
